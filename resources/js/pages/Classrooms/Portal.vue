...
const tabs = computed(() => [
    { id: 'wall', label: t('classrooms.portal.tabs.wall'), icon: MessageSquare },
    { id: 'activities', label: t('classrooms.portal.tabs.activities'), icon: ClipboardCheck },
    { id: 'materials', label: t('classrooms.portal.tabs.materials'), icon: FileText },
    { id: 'forum', label: t('classrooms.portal.tabs.forum'), icon: Users },
]);
...
const formatDate = (value: string): string => new Date(value).toLocaleString(locale.value === 'pt' ? 'pt-BR' : 'en-US');
</script>
        ...
        <main class="mx-auto w-full max-w-6xl flex-1 px-3 py-5 sm:px-5 md:py-9">
            <header class="relative overflow-hidden rounded-3xl bg-slate-950 p-6 text-white shadow-xl md:p-10">
                ...
                <div class="relative max-w-3xl">
                    <p class="text-xs font-black tracking-[.22em] uppercase" :style="{ color: accent }">{{ classroom.church?.name }} · {{ t('classrooms.portal.kicker') }}</p>
                    ...
                    <Link v-if="canManage" :href="`/dashboard/classrooms/${classroom.id}/content`" class="mt-5 ml-4 inline-flex items-center gap-2 rounded-full bg-white/10 px-4 py-2 text-sm font-bold backdrop-blur hover:bg-white/20"><Settings class="size-4" />{{ t('classrooms.portal.manage') }}</Link>
                </div>
            </header>
            ...
            <section v-if="active === 'wall'" class="mt-6 space-y-6">
                ...
                <div v-if="!wallPosts.length" class="rounded-3xl border border-dashed border-slate-300 bg-white p-12 text-center"><MessageSquare class="mx-auto size-9 text-slate-300" /><p class="mt-3 font-bold text-slate-600">{{ t('classrooms.portal.wall_empty') }}</p></div>
            </section>
            ...
            <section v-else-if="active === 'activities'" class="mt-6 space-y-5">
                <article v-for="activity in activities" :key="activity.id" class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm md:p-7">
                    <div class="flex flex-wrap items-start justify-between gap-3">
                        <div><p class="text-xs font-black tracking-wider uppercase" :style="{ color: accent }">{{ t('classrooms.portal.activity_meta', { attempts: activity.attempts, max: activity.max_attempts }) }}</p><h2 class="mt-1 text-xl font-black">{{ activity.title }}</h2><p v-if="activity.instructions" class="mt-2 text-sm leading-6 text-slate-600">{{ activity.instructions }}</p></div>
                        <span v-if="activity.available_until" class="rounded-full bg-amber-50 px-3 py-1 text-xs font-bold text-amber-800">{{ t('classrooms.portal.available_until', { date: formatDate(activity.available_until) }) }}</span>
                    </div>
                    ...
                    <div class="mt-5 flex justify-end"><button type="button" :disabled="!activity.can_submit || submitting === activity.id" class="rounded-full px-6 py-2.5 text-sm font-black text-white disabled:opacity-40" :style="{ backgroundColor: accent }" @click="submitActivity(activity)">{{ activity.can_submit ? t('classrooms.portal.submit_activity') : t('classrooms.portal.limit_reached') }}</button></div>
                </article>
                <p v-if="!activities.length" class="rounded-3xl border border-dashed border-slate-300 bg-white p-12 text-center text-slate-500">{{ t('classrooms.portal.activities_empty') }}</p>
            </section>
            ...
            <section v-else-if="active === 'materials'" class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <p v-if="!materials.length" class="rounded-3xl border border-dashed border-slate-300 bg-white p-12 text-center text-slate-500 sm:col-span-2 lg:col-span-3">{{ t('classrooms.portal.materials_empty') }}</p>
            </section>
            ...
            <section v-else class="mt-6 space-y-5">
                <form v-if="canUseForum" class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm md:p-7" @submit.prevent="createTopic">
                    <h2 class="text-xl font-black">{{ t('classrooms.portal.start_discussion') }}</h2>
                    <div class="mt-4 grid gap-3"><input v-model="topic.title" required maxlength="180" class="rounded-xl border-slate-300"  :placeholder="t('classrooms.portal.subject')" /><textarea v-model="topic.content" required maxlength="10000" rows="4" class="rounded-xl border-slate-300"  :placeholder="t('classrooms.portal.discussion_placeholder')" /></div>
                    <div class="mt-4 flex justify-end"><button class="inline-flex items-center gap-2 rounded-full px-5 py-2.5 text-sm font-black text-white" :style="{ backgroundColor: accent }"><Send class="size-4" />{{ t('classrooms.portal.publish') }}</button></div>
                </form>
                <article v-for="discussion in discussions" :key="discussion.id" class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm md:p-7">
                    ...
                    <form v-if="canUseForum && !discussion.is_locked" class="mt-4 flex gap-2" @submit.prevent="reply(discussion)"><input v-model="replies[discussion.id]" required class="min-w-0 flex-1 rounded-xl border-slate-300 text-sm"  :placeholder="t('classrooms.portal.reply_placeholder')" /><button class="grid size-11 place-items-center rounded-xl text-white" :style="{ backgroundColor: accent }"><Send class="size-4" /></button></form>
                </article>
                <p v-if="!discussions.length" class="rounded-3xl border border-dashed border-slate-300 bg-white p-12 text-center text-slate-500">{{ t('classrooms.portal.forum_empty') }}</p>
            </section>
        </main>
