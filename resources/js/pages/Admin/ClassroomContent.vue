...
<template>
    ...
    <main class="mx-auto max-w-7xl space-y-6 px-4 py-8">
        ...
        <section v-else-if="tab === 'activities'" class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_22rem]">
            <div class="space-y-3"><article v-for="item in classroom.activities" :key="item.id" class="rounded-2xl border bg-white p-5"><div class="flex justify-between"><div><h3 class="font-black">{{ item.title }}</h3><p class="text-sm text-slate-500">{{ item.form?.title }} · {{ t('classrooms.admin.attempt_count', { count: item.max_attempts }) }}</p></div><button class="text-rose-600" @click="remove(`${base}/activities/${item.id}`)"><Trash2 class="size-4" /></button></div></article></div>
        ...
        <section v-else-if="tab === 'materials'" class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_22rem]">
            <div class="grid gap-3 md:grid-cols-2"><article v-for="item in classroom.materials" :key="item.id" class="rounded-2xl border bg-white p-5"><div class="flex justify-between"><div><h3 class="font-black">{{ item.title }}</h3><p class="text-xs text-slate-500">{{ t(`classrooms.admin.${item.type}`) }}</p></div><button class="text-rose-600" @click="remove(`${base}/materials/${item.id}`)"><Trash2 class="size-4" /></button></div></article></div>
